...
void setLimits(int maxProccessorTime, int maxMemory);
void raportUsage();
...
int main(int argc, char* args[]) {
    ...
    int maxProccessorTime = atoi(args[5]);
    int maxMemory = atoi(args[6]);
    ...
    if (strcmp(args[4], "paste") == 0) {
        ...
        for (int i = 0; i < numberOfWorkers - 1; i++) {
            ...
            if (workerPID == 0) {
                ...
                setLimits(maxProccessorTime, maxMemory);
...
void printHelp() {
    ...
    printf("-> ./macierz _file_list _number_of_subprocesses _max_time_for_subproccess _how_to_make_final_file _max_proccessor_time _max_memory\n\n");
}
...
void checkArgs(int argc, char* args[]) {
    ...
    if (atoi(args[2]) <= 0 || atoi(args[3]) <= 0 || atoi(args[5]) <= 0 || atoi(args[6]) <= 0) {
...
void setLimits(int maxProccessorTime, int maxMemory) {
    struct rlimit* limit = (struct rlimit *)calloc(1, sizeof(struct rlimit));
    limit->rlim_cur = maxProccessorTime;
    limit->rlim_max = maxProccessorTime;
...
void raportUsage() {
    ...
    printf("Proceess PID = %i:\n", getpid());
    printf("-> maximum resident set size: %ld KB\n", usage->ru_maxrss);
    printf("-> user CPU time: %ld KB\n", usage->ru_utime.tv_usec);
    printf("-> system CPU time: %ld KB\n", usage->ru_stime.tv_usec);
    printf("\n");

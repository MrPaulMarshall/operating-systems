...
int main(int argc, char * args[]) {
    ...
    // here goes game
    if (sendMsg(sockfd, '\0', args[1]) < 0) error("Error writing to socket");
    ...
    if (ifInUse(msgType)) {
        ...
        do {
            if (myTurn == 1) {
                if (getterThrActive == 0) {
                    ...
                    if (inputReady == 1) {
                        ...
                        if (sendMsg(sockfd, input, NULL) < 0) error("Write field to socket");
                    }
                }
            }
            ...
            if (msgType == serverPing) {
                if (sendMsg(sockfd, clientPing, NULL) < 0) error("Write ping to socket");
            }
...
void h_sigint(int signal) {
    ...
    sendMsg(sockfd, clientExit, NULL);
    shutdown(sockfd, SHUT_RDWR);
    close(sockfd);

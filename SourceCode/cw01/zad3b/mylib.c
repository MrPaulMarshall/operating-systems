...
void appendToString(String* string, char* str) {
    while(string->maxSize <= string->size + strlen(str)) {
        string->maxSize *= 2;
    }
...
FilePair* createFilePair(char* file1, char* file2) {
    ...
    pair->file1 = strdup(file1);
    pair->file2 = strdup(file2);
...
void printPair(FilePair* pair) {
    printf("pair: f1 = %s, f2 = %s\n", pair->file1, pair->file2);
}
...
void compareFiles(FilePair* pair) {
    // przykladowa komenda:
...
void loadOperationsFromFile(Block* block) {
    ...
    for(i = 0; i < numberOfOperations; i++) {
        block->diffResult[i] = strdup(strArr[i]->string);
    }

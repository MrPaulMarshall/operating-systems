...
BlocksArray* execCreateBlocksArray(char* sizeAsString) {
    return createBlocksArray( atoi(sizeAsString) );
}
...
int main(int argc, char* args[]) {
    ...
    FILE* resultFile = fopen("./results3a.txt", "a");
    if(resultFile == NULL) {
    ...
    for(i = 1; i < argc; i++) {
        ...
        if(compareStr(args[i], "show") == 0) {
            ...
            blocksArray = execCreateBlocksArray(args[i+1]);
            i++;
